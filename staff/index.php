...
if(isset($_POST['log'])) {
    ...
    $sql = "SELECT u.*, e.* FROM users u, emp_details e WHERE u.username=? AND e.id=u.id";
    $stmt = mysqli_prepare($con, $sql);
    mysqli_stmt_bind_param($stmt, "s", $username); // Bind username instead of email
    ...
    if(mysqli_num_rows($result) > 0) {
        ...
        if(password_verify($pass, $row["password"])) {
            if($row["status"] == "Active") {
                ...
                $insert_sql = "INSERT INTO emp_history VALUES(?, ?, 'logged still')";
                $insert_stmt = mysqli_prepare($con, $insert_sql);
                mysqli_stmt_bind_param($insert_stmt, "ss", $idd, $tms1);
    ...
    <body>
        <form class="login-form"  method="POST">
            <div class="login-form__logo-container">
                <h3 style="color: white; text-align: center;">Gold Coast Central Bank</h3>
            </div>
            <div class="login-form__content">
                ...
                <input class="login-form__input" type="text" name="username" placeholder="Username" required autofocus>
