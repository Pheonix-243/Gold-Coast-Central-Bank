...
// Build query
$query = "SELECT ah.*, a.account_title, a.account_type
...
// Calculate totals
$totalQuery = "SELECT SUM(ah.amount) as total_interest, COUNT(*) as transaction_count
               FROM account_history ah
               JOIN accounts_info a ON ah.account = a.account
               WHERE ah.type = 8
               AND ah.dt BETWEEN ? AND ?";
...
if (!empty($accountFilter)) {
    $totalQuery .= " AND ah.account LIKE ?";
...
<head>
    ...
    <title>Interest Posting Report</title>
        ...
        <div class="card mb-4">
            ...
            <div class="card-body">
                <form method="get" class="form-inline">
                    ...
                    <a href="interest_posting_report.php" class="btn btn-secondary ml-2">Reset</a>
                </form>
            </div>
        </div>
        ...
        <div class="card">
            ...
            <div class="card-body">
                <?php if ($result->num_rows > 0): ?>
                            ...
                            <tbody>
                                <?php while ($row = $result->fetch_assoc()): ?>
                                    <tr>
                                        ...
                                        <td><?php echo number_format($row['balance_after'], 2); ?></td>
                    ...
                    <div class="mt-3">
                        <a href="export_report.php?type=posting&start_date=<?php echo $startDate; ?>&end_date=<?php echo $endDate; ?>&account=<?php echo urlencode($accountFilter); ?>&account_type=<?php echo urlencode($accountTypeFilter); ?>" 
                           class="btn btn-success">
                            Export to CSV
                        </a>
                    </div>
                <?php else: ?>
                    <p>No interest postings found for the selected period.</p>
                <?php endif; ?>
            </div>
        </div>
    </div>
