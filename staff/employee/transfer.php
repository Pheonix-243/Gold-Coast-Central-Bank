...
// - Email functionality (email.php)
// - Custom library functions (library.php)
include '../../conn.php';
include '../../email.php';
include 'library.php';
...
// Check if the form was submitted with the 'transfer' button
if (isset($_POST['transfer'])){
    // Get sender's details from the form
    $name = $_POST['name'];         // Sender's name
    $acc = $_POST['sender2'];       // Sender's account number
    $email = $_POST['email'];       // Sender's email
    $title = $_POST['sender1'];     // Sender's account title
    $blnc = $_POST['blnc'];         // Sender's current balance
    ...
    // Check if transfer amount exceeds sender's balance
    if($newbnc > $blnc){
        // Set error message if balance is insufficient
        $_SESSION["title"] = "Error";
        $_SESSION["status"] = "Current Balance is low";
        $_SESSION["code"] = "error";
    } else {
        // Calculate new balances after transfer
        $bnc1 = $blnc - $newbnc;  // Sender's new balance
        $bnc2 = $blnc1 + $newbnc; // Receiver's new balance
        
        // Prepare SQL queries to update both accounts
        $query = "UPDATE accounts_info set balance='$bnc1' where account='$acc'";      // Update sender's balance
        $query1 = "UPDATE accounts_info set balance='$bnc2' where account='$acc1'";   // Update receiver's balance
        ...
        if($rs1 && $rs2){
            ...
            $tms = date("h:i:s");             // Current time
            $tms1 = date("Y-m-d h:i:s");      // Current date and time
            ...
            if ($connected){
                // Prepare and send email to sender
                $msg = "Hello dear ".$name."! You have made transection on ".$tms1." from SKY BANK ACCOUNT that is done successfully. Amount ".$newbnc.".00$ is send to ".$acc1." successfully. Your remaining account balance is ".$bnc1.".00$. Thank you for joining SKY BANK service.";
                email_send($email, "Transection done successfully", $msg);
                
                // Prepare and send email to receiver
                $msg1 = "Hello dear ".$name1."! You have recieved amount ".$newbnc.".00$ on ".$tms1." in your SKY BANK ACCOUNT from account nunmber ".$acc.". Your current account balance is ".$bnc2.".00$. Thank you for joining SKY BANK service.";
            ...
            // Set error message if transfer failed
            $_SESSION["title"] = "Error";
