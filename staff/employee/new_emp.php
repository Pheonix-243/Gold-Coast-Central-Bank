...
if (isset($_POST['insert'])) {
    ...
    // Check for duplicates using prepared statements
    $sqli_qery = "SELECT e.cnic, e.email, u.username from emp_details e, users u WHERE u.username=? OR e.cnic=? OR e.email=?";
    $stmt = mysqli_prepare($con, $sqli_qery);
    mysqli_stmt_bind_param($stmt, "sss", $username, $cnic, $email);
    mysqli_stmt_execute($stmt);
    $dupli = mysqli_stmt_get_result($stmt);
    $duplicate = mysqli_fetch_array($dupli, MYSQLI_ASSOC);
    
    $dup_cnic = $duplicate["cnic"];
    $dup_email = $duplicate["email"];
    $dup_user = $duplicate["username"];
    ...
    if($dup_cnic == $cnic) {
        ...
        // Insert employee details with prepared statement
        $sql = "INSERT INTO emp_details(id, name, fname, cnic, contect, dob, gender, marital, email, image, postal, city, houseaddress, edu, title, exp, hier_date) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
        $stmt = mysqli_prepare($con, $sql);
        mysqli_stmt_bind_param($stmt, "sssssssssssssssss", $acc_full, $name, $fname, $cnic, $contect, $dob, $gender, $marry, $email, $imgData, $postal, $city, $address, $edu, $title, $experience, $regisdate);
        $current_id = mysqli_stmt_execute($stmt);
        
        if ($current_id) {
            mysqli_query($con, "UPDATE counting set emp_id='$acc_num'");
            
            // Insert user with hashed password using prepared statement
            $sqli = "INSERT INTO users(id, username, password, role, status) VALUES (?, ?, ?, ?, 'Active')";
            $stmt = mysqli_prepare($con, $sqli);
            mysqli_stmt_bind_param($stmt, "ssss", $acc_full, $username, $hashed_password, $type);
            mysqli_stmt_execute($stmt);
            ...
            if ($connected) {
                $msg = "Hello dear ".$name."! Your SKY BANK account is created successfully at ".$tms1.". Your Username: '".$username."' and Password: '".$pass."'. Use given username and password for account login. Thank you for joining SKY BANK service.";
...
<nav class="navbar">
    ...
    <section class="content" id="top">
       <div class="container-fluid">
            ...
            <div  class="col-sm-4">
                <input type="text" class="form-control" name="gender" value="<?php if($_SESSION['type']=="Default") { echo "ADN".$acc_num."  |  EMP".$acc_num;} else{ echo "EMP".$acc_num;} ?>" readonly>
            </div>
            </div>
            <!-- Widgets -->
            ...
            <div class="row clearfix">
                   ...
                   <div class="box-body">
                        ...
                        <div class="row">
                            ...
                            <div class="col-lg-4">
                                <p for="exampleInputPassword1" style="margin-bottom: 1px; margin-top: 8px;">Marital Status</p>
                        ...
                        <div class="row">
                            <div class="col-lg-4">
                                ...
                                <input type="text" class="form-control" name="edu" placeholder="Enter your education" required>
                            </div>
                            <div class="col-lg-4">
                                <p for="exampleInputPassword1" style="margin-bottom: 1px; margin-top: 8px;">Current Job Title</p>
                                <input type="text" class="form-control" id="title" name="title" placeholder="Enter current job title" required>
                            </div>
                           <div class="col-lg-4">
                                <p for="exampleInputPassword1" style="margin-bottom: 1px; margin-top: 8px;">Experience</p>
                                <select class="form-control" name="experience" required>
                                  <option selected disabled hidden value="">Select your experience</option>
                                  ...
                                  <option value="Above then 5 Year">Above then 5 Year</option>
                                </select>
                            </div>
                        </div>
                        <hr style="height:1px;border-width:0; width: 100%; margin-bottom:-5px; color:red;background-color:gray;">
                        ...
                        <div class="row">
                            <div class="col-lg-4">
                                ...
                                <input type="email" class="form-control" name="username" placeholder="Enter username for login" required>
                            </div>
                           <div class="col-lg-4">
                                <p for="exampleInputPassword1" style="margin-bottom: 1px; margin-top: 8px;">User Type</p>
                                <select class="form-control" name="user_type" required>
                                  <option selected disabled hidden value="">Select User type</option>
