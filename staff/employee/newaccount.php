...
$acc_num=$acc_num+1;
$acc_full="GH10".$acc_num;
$pass = OTP(8); // Generate 8-digit password
...
if (isset($_POST['insert'])){
    ...
    $sqli_qery="SELECT cnic,email from accountsholder where cnic=? or email=?";
    ...
    if($dup_cnic==$cnic){
        ...
        // Insert account holder details with prepared statement
        $sql= "INSERT INTO accountsholder(account,name,fname,cnic,contect,dob,gender,email,image,postal,city,houseaddress) VALUES (?,?,?,?,?,?,?,?,?,?,?,?)";
        $stmt = mysqli_prepare($con, $sql);
        mysqli_stmt_bind_param($stmt, "ssssssssssss", $acc_full, $name, $fname, $cnic, $contect, $dob, $gender, $email, $imgData, $postal, $city, $address);
        $current_id = mysqli_stmt_execute($stmt);
        
        if (isset($current_id)) {
            mysqli_query($con,"UPDATE counting set acc_count='$acc_num'");
            
            // Insert account info with prepared statement
            $sqli = "INSERT INTO accounts_info(account,account_title,account_type,balance,registerdate,password,status) VALUES (?,?,?,?,?,?,'Active')";
            $stmt = mysqli_prepare($con, $sqli);
            mysqli_stmt_bind_param($stmt, "ssssss", $acc_full, $title, $type, $balance, $regisdate, $hashed_password);
            mysqli_stmt_execute($stmt);
            ...
            if ($connected){
                $msg="Hello dear ".$name."! Your SKY BANK account is created successfully at ".$tms1.". Your Account Number: '".$acc_full."' and Password: '".$pass."'. Use your account number and password for login. You have deposited ".$balance.".00$. Your current account balance is ".$balance.".00$. Thank you for joining SKY BANK service.";
