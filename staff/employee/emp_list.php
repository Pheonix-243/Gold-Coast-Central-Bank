    ...
    <section class="content" id="top">
       <div class="container-fluid">
            <div class="block-header">
                <p style="margin-left: -15px; font-size: 17px; font-weight: bold;">Accounts List</p>
            </div>
            ...
            <br>
                <div class="row">
                <div class="col-sm-4">
                    <input type="text" id="myInput" class="cs form-control" onkeyup="myFunction()" placeholder="Search from table with id, name and cnic" title="Type in a name">
                </div>
                <?php 
                    ...
                    <table class="table" id="myTable">
                      <thead>
                        <tr style="text-align: center;" class="tb">
                          ...
                          <th style="text-align: center; width: 14%;">Employee Id</th>
                          <th style="text-align: center; width: 18%;">Name</th>
                          <th style="text-align: center; width: 14%;">CNIC Number</th>
                          <th style="text-align: center; width: 14%;">Contect</th>
                          <th style="text-align: center; width: 11%;">Employee Type</th>
                      ...
                      <tbody id="body">
                        ...
                        if(isset($_POST['filter'])){
                            ...
                            if($_SESSION['type']=="Default"){
                            $sql="SELECT u.*,e.* from users u, emp_details e where u.id=e.id and e.hier_date between '$startdt' and '$enddt' order by e.hier_date DESC";
                          }else{
                          $sql="SELECT u.*,e.* from users u, emp_details e where u.id=e.id and u.role='Employee' and e.hier_date between '$startdt' and '$enddt' order by e.hier_date DESC";
                         }
                        ...
                        <tr style="height: 50px;">
                           ...
                           <td style="text-align: center;padding-top: 15px;"><div class="action_btn"><a href="emp_profile.php?id=<?php echo $row['id'];?>" class="click_ripple_shine success" title="View"><i class="icon fa fa-eye"></i> </a>
                            <?php
                            if ($row['status']=="Active") {
                            ?>
                            <a href="emp_block.php?id=<?php echo $row['id'];?>& type=Block" class="dng1"title="Block Account"><i class="icon fa fa-ban"></i></a>
                            <?php  } ?>
                            <?php
                            if ($row['status']=="Block") {
                            ?>
                            <a href="emp_block.php?id=<?php echo $row['id'];?>& type=Active" class="success dng"title="Activate Account"><i class="icon fa fa-check"></i></a>
                            <?php  } ?>
                           <a href="emp_update.php?id=<?php echo $row['id'];?>" class="click_ripple_shine warning" title="Edit"><i class="icon fa fa-pencil-square-o"></i></a>
                           <a href="emp_delete.php?id=<?php echo $row['id'];?>" class="danger"title="Delete"><i class="icon fa fa-close"></i></a>
                           </div>
                       </td>
                        <?php
                         }
                        ...
                        if($_SESSION['type']=="Default"){
                        $sql="SELECT u.*,e.* from users u, emp_details e where u.id=e.id order by e.hier_date DESC";
                        }else{
                          $sql="SELECT u.*,e.* from users u, emp_details e where u.id=e.id and u.role='Employee' order by e.hier_date DESC";
                        }
                        $result = mysqli_query($con,$sql);
                        $num = (mysqli_query($con,$sql));
                         if(mysqli_num_rows($num)>0){
...
<div class="row clearfix" id="not">
    ...
    <script type="text/javascript">
       $('.dng').on('click', function(e){
        ...
        Swal.fire({
  title: 'Confirm to activate account?',
  icon: 'warning',
  ...
  if (result.isConfirmed) {
    document.location.href=href;
       ...
       $('.dng1').on('click', function(e){
        ...
        const href=$(this).attr('href');
        Swal.fire({
  title: 'Confirm to Block account?',
  icon: 'warning',
  showCancelButton: true,
